    ...
    <>
      ...
      <div className="mt-2 max-w-4xl mx-auto pt-2 pl-3">
        ...
        <ol className="flex gap-2 mt-3 justify-start">
          <li className={`py-1 px-2 border-gray-1 border-2 rounded-[20px] ${isSelected === 'tumbler' ? 'bg-primary' : ''}`}>
            <button type="button" onClick={() => handleClickButton('tumbler')}>텀블러</button>
          </li>
          <li className={`py-1 px-2 border-gray-1 border-2 rounded-[20px] ${isSelected === 'pouch' ? 'bg-primary' : ''}`}>
            <button type="button" onClick={() => handleClickButton('pouch')}>파우치</button>
          </li>
          <li className={`py-1 px-2 border-gray-1 border-2 rounded-[20px] ${isSelected === 'echobag' ? 'bg-primary' : ''}`}>
            <button type="button" onClick={() => handleClickButton('echobag')}>에코백</button>
          </li>
          <li className={`py-1 px-2 border-gray-1 border-2 rounded-[20px] ${isSelected === 'zibits' ? 'bg-primary' : ''}`}>
